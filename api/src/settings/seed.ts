...
import { DEFAULTS, DEFAULT_GEMINI, DEFAULT_OLLAMA } from './defaults.js';
...
export async function seedFromEnv() {
  ...
  await seedSetting('extraction_model', process.env.EXTRACTION_MODEL ?? process.env.GEMINI_MODEL ?? DEFAULT_GEMINI.model);
  ...
  await seedCreds('gemini', {
    apiKey: process.env.GEMINI_API_KEY,
    model: process.env.GEMINI_MODEL ?? DEFAULT_GEMINI.model,
  });
}
