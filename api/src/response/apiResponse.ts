/**
 * Stable /api/parse response shape. Single source of truth for how parsed_data is normalized,
 * shared by the route and the DB verification script so they can never drift apart.
 */
import type {
...
// A bill is either inter-state (IGST) or intra-state (CGST+SGST), never both. A GST rate is only
// real when its amount is actually charged (> 0): a side with no GST (e.g. zero-labour) must not
// carry leftover inferred rates, and the response must never show "cgst 9 + igst 18 + sgst 9".
...
// Normalize parsed_data into the stable schema shape — every key present (null when missing),
// both line-item arrays, per-side GST rate+amount, deductibles/salvage.
