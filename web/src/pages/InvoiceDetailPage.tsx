...
function CostBreakdown({ inv }: { inv: Invoice }) {
  ...
  // and Analytics (which sums the stored value) can quietly disagree.
  ...
  // Non-zero when the provider's total covers tokens the components don't (e.g.
  // cached input). Surfaced rather than silently absorbed.
            ...
            {unaccountedTokens !== 0 && (
              <div style={{ ...rowStyle, paddingTop: 0, color: T.muted, fontSize: 11 }}>
                <span style={{ paddingLeft: 12 }}>
                  ...
                  by the provider beyond input + output (e.g. cached input) — not separately priced
                </span>
                <span />
              </div>
            )}
