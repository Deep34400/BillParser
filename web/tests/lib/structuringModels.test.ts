...
import { STRUCTURING_MODEL_SUGGESTIONS, modelForProvider, normalizeGeminiModel } from '../../src/lib/structuringModels.js';
...
it('upgrades retired Gemini model IDs to gemini-2.5-flash', () => {
  expect(normalizeGeminiModel('gemini-2.0-flash')).toBe('gemini-2.5-flash');
  expect(modelForProvider('gemini', 'gemini-2.0-flash')).toBe('gemini-2.5-flash');
});
