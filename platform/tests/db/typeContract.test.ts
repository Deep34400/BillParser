 ...
 *
 * 1. node-postgres returns NUMERIC (oid 1700) as a *string* by default. Left
 *    unhandled, `totalSpend += bill.grand_total_amount` becomes string
 *    concatenation and every analytics figure silently turns to garbage.
 *    src/config/db.ts registers a global parser; these tests prove it holds —
 *    including through raw `db.execute(sql...)`, which bypasses Drizzle's own
 *    column mapping.
 *
 * 2. Timestamps must surface as ISO strings, never Date objects, because the
 *    domain interfaces (BillDoc etc.) and the entire web app expect strings.
 */
import { describe, it, expect, beforeEach } from 'vitest';
import { sql } from 'drizzle-orm';
import { db } from '../../src/config/db.js';
import { bills, billParts, users, tokenTransactions } from '../../src/db/schema.js';
...
describe('NUMERIC columns surface as JS numbers', () => {
  ...
  it('supports arithmetic without string concatenation', async () => {
    ...
    // The actual bug this guards: '1000' + '200' === '1000200'
    const sum = (bill!.parts_amount ?? 0) + (bill!.labour_amount ?? 0);
    expect(sum).toBe(1200);
    expect(typeof sum).toBe('number');
  });

  it('holds through raw db.execute(), which bypasses Drizzle column mapping', async () => {
    await createBill(makeBill('b4'));
    const result = await db().execute(
      sql`SELECT SUM(grand_total_amount) AS total, AVG(parts_cgst_rate) AS avg_rate FROM bills`,
    );
    const row = result.rows[0] as { total: unknown; avg_rate: unknown };

    expect(typeof row.total).toBe('number');
    expect(typeof row.avg_rate).toBe('number');
...
describe('timestamps surface as ISO strings', () => {
  ...
  it('returns created_at/updated_at as strings, not Date objects', async () => {
    ...
    expect(new Date(bill!.created_at).toISOString()).toBe(bill!.created_at);
  });
});
    ...
    await saveBillParts([
      {
        ...
        normalized_name: null, confidence_score: null, created_at: '2026-07-01T00:00:00Z',
      },
      {
