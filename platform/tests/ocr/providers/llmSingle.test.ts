...
import { saveProviderCredentials } from '../../../src/shared/settings.js';
...
describe('llmSingle providers', () => {
  ...
  it('rejects OpenAI single for PDF', async () => {
    ...
    await saveProviderCredentials('openai', { apiKey: 'sk-test' });
  ...
  it('does not reject Mistral single for PDF with images-only error', async () => {
    const pdf = Buffer.from('%PDF-1.4 fake');
    await saveProviderCredentials('mistral', { apiKey: 'sk-test' });
