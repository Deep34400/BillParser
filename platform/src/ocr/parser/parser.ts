 ...
 *   3. Flat entries: { entries: [{ parsed_data }] }
 *   4. Top-level: { parsed_data: {...} }
 *   5. Flat root: { company_name, parts_line_items, ... }
 *   6. Legacy canonical: { vendorName, lineItems, ... }
 */
import type {
  ParsedInvoiceData, PartsLineItem, LabourServiceLineItem,
  ServiceDetails, VehicleDetails, TotalsAndTaxSummary,
} from '../types/invoice.js';
import type { ParseResult, ValidationIssue } from '../types/parser.js';
...
// ── Type coercion for ParsedInvoiceData sub-objects ─────────────
...
function parseLegacyCanonical(raw: string, markdown?: string): Omit<CanonicalResult, 'rawText' | 'rawJson'> {
