 ...
 *   transformer/normalize/vendor.ts   — seller/buyer detection + correction
 *   transformer/normalize/date.ts     — date normalization + fallback
 *   transformer/normalize/footer.ts   — GST/discount/deductible from markdown
 *   transformer/normalize/index.ts    — line-item enrichment pipeline
 *   transformer/review.ts             — human-review warnings
 *   transformer/validate.ts           — structural validation
 *
 * Adding new invoice formats? Fix in CODE (with tests), not here.
 */

/** Minified skeleton — smaller/faster for structuring prompts. */
const SCHEMA_JSON_SKELETON =
